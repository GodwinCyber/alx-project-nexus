...
function ProductSubCategorySelect({ categoryId, value, onChange }: { 
  categoryId: string; 
  value: string; 
  ...
  const subCategories = (subCategoriesData as any)?.allSubCategories?.edges || [];
...
export function AdminPage() {
  ...
  const { data: subCategoriesData, refetch: refetchSubCategories } = useQuery(GET_SUB_CATEGORIES, {
    variables: { categoryId: selectedCategoryId ? parseInt(selectedCategoryId) : undefined },
  ...
  const [createSubCategory, { loading: creatingSubCategory }] = useMutation(CREATE_SUB_CATEGORY, {
    onCompleted: (data: any) => {
      if (data.createSubCategory.ok) {
  ...
  const handleDeleteCategory = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this category? This will also delete all associated sub-categories and products.')) {
  ...
  const handleUpdateSubCategory = async (id: string, name: string, categoryId: string) => {
    if (!name.trim()) {
                      ...
                      <>
                        ...
                        <div className="flex space-x-1">
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => setEditingCategory({id: edge.node.id, name: edge.node.name})}
                          >
          ...
          <CardContent>
            <form onSubmit={handleCreateSubCategory} className="space-y-4">
              ...
              <Input
                ...
                required
              />
              <Button type="submit" className="w-full" disabled={creatingSubCategory}>
                <Plus className="mr-2 h-4 w-4" />
                    ...
                    >
                      {editingSubCategory?.id === edge.node.id ? (
                        <div className="flex items-center space-x-2 flex-1">
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleUpdateSubCategory(editingSubCategory.id, editingSubCategory.name, editingSubCategory.categoryId)}
                          >
                            <Save className="h-3 w-3" />
                          </Button>
                          <Button
                        ...
                        <>
                          ...
                          <div className="flex space-x-1">
                            ...
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => handleDeleteSubCategory(edge.node.id)}
                            >
                              <Trash2 className="h-3 w-3 text-destructive" />
                            </Button>
                          </div>
          ...
          <CardContent>
            <form onSubmit={handleCreateProduct} className="space-y-4">
              ...
              <Input
                ...
                onChange={(e) => setProductData(prev => ({ ...prev, amountInStock: e.target.value }))}
                required
              />
              <select
                value={productData.categoryId}
                onChange={(e) => setProductData(prev => ({ ...prev, categoryId: e.target.value, subCategoryId: '' }))}
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                required
              >
                <option value="">Select Category</option>
              ...
              {productData.categoryId && (
                <ProductSubCategorySelect
                  ...
                  onChange={(value) => setProductData(prev => ({ ...prev, subCategoryId: value }))}
                />
              )}
              <Button type="submit" className="w-full" disabled={creatingProduct}>
                <Plus className="mr-2 h-4 w-4" />
