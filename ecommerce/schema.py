...
class ProductImageInput(graphene.InputObjectType):
    ...
    image = Upload(
        required=True
    )  # Use graphene-file-upload’s scalar to accept multipart file data
...
class UpdateCategory(graphene.Mutation):
    ...
    class Arguments:
        id = graphene.Int(required=True)  # <-- explicit id arg
        input = CategoryInput(required=True)
...
class CreateProductImage(graphene.Mutation):
    ...
    def mutate(root, info, input):
        ...
        product_image = ProductImage(product=product, image=input.image)
        product_image.save()
...
class CreateOrder(graphene.Mutation):
    ...
    def mutate(root, info, status="created"):
        ...
        with transaction.atomic():
            ...
            order = Order(user=user, status=status, total=Decimal("0.00"))
            order.save()
    ...
    def mutate(root, info, input):
        ...
        try:
            intent = stripe.PaymentIntent.create(
                amount=int(Decimal(input.amount) * Decimal(100)).quantize(
                    Decimal("1")
                ),  # amount in cents
        ...
        return CreatePayment(
            payment=payment, client_secret=intent["client_secret"], ok=True
        )
